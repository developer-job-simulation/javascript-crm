...
export const makeTable = async () => {
  ...
  companies.map((company) => {
    ...
    row.push(
      ...
      new Date(company[CREATED_AT_FIELD_NAME]).toLocaleTimeString().slice(0, 5),
  ...
  companiesToDisplay.forEach((row, idx) => {
    const tr = idx === 0 ? header.insertRow() : body.insertRow() //Create a new row
